    ...
    A: PartialEq + Eq + Hash,
{
    ...
    /// Check whether e1 and e2 are known to be equal, without entering
    /// either of them: a query that misses leaves the structure untouched.
    pub(crate) fn known_eq(&mut self, e1: &A, e2: &A) -> bool {
        match (self.node_map.get_index_of(e1), self.node_map.get_index_of(e2)) {
            (Some(i1), Some(i2)) => self.find_parent_idx(i1) == self.find_parent_idx(i2),
            _ => false,
        }
    }
