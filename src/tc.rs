...
impl<'x, 't: 'x, 'p: 't> TypeChecker<'x, 't, 'p> {
    ...
    /// input.
    pub(crate) fn whnf_clo(&mut self, c: Clo<'t>) -> SClo<'t> {
        let d = self.probe_suspend();
        let r = stacker::maybe_grow(256 * 1024, 16 * 1024 * 1024, || self.whnf_clo_inner(c));
        self.probe_resume(d);
        r
    }

    fn whnf_clo_inner(&mut self, c: Clo<'t>) -> SClo<'t> {
        let c = self.norm_clo(c);
        ...
        let t = self.whnf_core_clo(c);
        let r = self.whnf_loop(t);
    ...
    pub(crate) fn whnf_sclo(&mut self, s: SClo<'t>) -> SClo<'t> {
        self.ctx.rp.ctrs[2] += 1;
        let t = self.whnf_core(s);
        self.whnf_loop(t)
    }

    fn whnf_loop(&mut self, mut t: SClo<'t>) -> SClo<'t> {
        for _ in 0..100_000u32 {
            if let Some(t2) = self.reduce_nat(&t) {
                t = self.whnf_core(t2);
                continue;
            }
            match self.unfold_definition(&t) {
                Some(t2) => t = self.whnf_core(t2),
                None => return t,
            }
        }
    ...
    pub(crate) fn whnf_core(&mut self, s: SClo<'t>) -> SClo<'t> {
        self.whnf_core_ext(s, false, false)
    }
    ...
    fn whnf_core_clo(&mut self, c: Clo<'t>) -> SClo<'t> {
        let c = self.norm_clo(c);
        match self.ctx.read_expr(c.e) {
            NatLit { .. } | StringLit { .. } | Sort { .. } | Pi { .. } | Lambda { .. }
            | Local { .. } => return SClo { head: c, spine: SpineVec::new() },
    ...
    fn whnf_core_ext(&mut self, s: SClo<'t>, cheap_rec: bool, cheap_proj: bool) -> SClo<'t> {
            ...
            match self.ctx.read_expr(e) {
                ...
                Lambda { body, .. } => {
                    if let Some(arg) = rsp.pop() {
                        env = self.push_entry(env, Entry::Val(arg.e, arg.env));
                        e = body;
                    } else {
                        break;
                    }
                ...
                Let { val, body, .. } => {
                    env = self.push_entry(env, Entry::Val(val, env));
                    e = body;
                }
                Proj { idx, structure, .. } => {
                    let sb = if cheap_proj {
                        let s0 = self.mk_sclo(Clo { e: structure, env });
                        self.whnf_core_ext(s0, cheap_rec, cheap_proj)
                    } else {
                        self.whnf_clo(Clo { e: structure, env })
                    };
                    let sb = self.lit_to_ctor(sb);
                ...
                Const { name, .. } => {
                    ...
                    let head = Clo { e, env };
                    if let Some(r) = self.reduce_recursor(head, &rsp, cheap_rec, cheap_proj) {
                        e = r.head.e;
                        env = r.head.env;
                        rsp = r.spine;
                        rsp.reverse();
    ...
    fn delta_hint(&self, e: ExprPtr<'t>) -> Option<ReducibilityHint> {
    ...
    fn unfold_definition(&mut self, s: &SClo<'t>) -> Option<SClo<'t>> {
    ...
    fn mk_nat_sclo(&mut self, n: BigUint) -> SClo<'t> {
        let e = self.ctx.mk_nat_lit_quick(n).unwrap();
        SClo { head: Clo::of(e), spine: SpineVec::new() }
    }

    fn mk_bool_sclo(&mut self, b: bool) -> Option<SClo<'t>> {
    ...
    /// Shared by `reduce_nat` and lazy-delta's cheap pre-test.
    fn nat_op(&self, s: &SClo<'t>) -> Option<NatOp> {
    ...
    fn reduce_nat(&mut self, s: &SClo<'t>) -> Option<SClo<'t>> {
        if !self.ctx.export_file.config.nat_extension {
            return None;
        }
        use NatOp::*;
        let op = self.nat_op(s)?;
        if op == Succ {
            let v = self.nat_lit(s.spine[0])?;
            return Some(self.mk_nat_sclo(v + 1u32));
        }
        // exponent/shift bound is checked before the base is forced
        let b = self.nat_lit(s.spine[1])?;
        if matches!(op, Pow | Shl) && b > BigUint::from(1u32 << 24) {
            return None;
        }
        let a = self.nat_lit(s.spine[0])?;
        let r = match op {
            Succ => unreachable!(),
            Beq => return self.mk_bool_sclo(a == b),
            Ble => return self.mk_bool_sclo(a <= b),
            Add => a + b,
            Sub => nat_sub(a, b),
            Mul => a * b,
    ...
    fn reduce_recursor(
        ...
        if matches!(self.env.get_declar(&fname), Some(Declar::Quot { .. })) {
            ...
            if let Some((mk_pos, arg_pos)) = pos {
                if mk_pos < len {
                    let mk = if cheap_rec {
                        let s0 = self.mk_sclo(idx(mk_pos));
                        self.whnf_core_ext(s0, cheap_rec, cheap_proj)
                    } else {
                        self.whnf_clo(idx(mk_pos))
                    };
                    let is_mk = matches!(self.ctx.read_expr(mk.head.e),
                        Const { name, .. } if Some(name) == nc.quot_mk)
                        && mk.spine.len() == 3;
                    if is_mk {
                        let m = self.mk_sclo(idx(arg_pos));
                        let mut spine = m.spine;
        ...
        let mut mj = if cheap_rec {
            let s0 = self.mk_sclo(idx(major_idx));
            self.whnf_core_ext(s0, cheap_rec, cheap_proj)
        } else if is_k && major_induct.is_some() {
            let s0 = self.mk_sclo(idx(major_idx));
            let conv = self.to_ctor_when_k(num_params, major_induct.unwrap(), s0.clone());
            if conv.head == s0.head && conv.spine == s0.spine {
                self.whnf_clo(idx(major_idx))
            } else {
                self.whnf_sclo(conv)
            }
        } else {
        ...
        mj = self.lit_to_ctor(mj);
        if !cheap_rec {
            if let Some(mi) = major_induct {
                mj = self.to_ctor_when_struct(mi, mj);
            }
        }
    ...
    fn to_ctor_when_k(
        ...
        let ty = self.infer_s(&mj, InferOnly);
        let app_type = self.whnf_clo(Clo::of(ty));
        ...
        let new_ty = self.infer_s(&new_ctor, InferOnly);
        let app_clo = self.sclo_as_clo(&app_type);
        if self.is_def_eq(app_clo, Clo::of(new_ty)) {
            return new_ctor;
        }
        mj
    }

    fn to_ctor_when_struct(&mut self, induct: NamePtr<'t>, mj: SClo<'t>) -> SClo<'t> {
        ...
        let ty = self.infer_s(&mj, InferOnly);
        let e_type = self.whnf_clo(Clo::of(ty));
        ...
        let tyty = self.infer_s(&e_type, InferOnly);
        let tyty_w = self.whnf_clo(Clo::of(tyty));
        ...
        let mj_clo = self.sclo_as_clo(&mj);
        ...
        for fi in 0..num_fields {
            ...
            let env = self.push_entry(ENV_NIL, Entry::Val(mj_clo.e, mj_clo.env));
    ...
    fn sclo_as_clo(&mut self, s: &SClo<'t>) -> Clo<'t> {
        if s.spine.is_empty() {
            return s.head;
        }
        let mut env = self.push_entry(ENV_NIL, Entry::Val(s.head.e, s.head.env));
    ...
    fn probe_enter(&mut self, budget: u64) -> (u64, bool, usize, usize) {
        let saved = (
    ...
    fn probe_exit(&mut self, saved: (u64, bool, usize, usize)) -> bool {
    ...
    #[inline]
    fn in_probe(&self) -> bool { self.ctx.rp.probe_depth > 0 }
    ...
    #[inline]
    fn probe_suspend(&mut self) -> u32 {
        std::mem::replace(&mut self.ctx.rp.probe_depth, 0)
    }

    #[inline]
    fn probe_resume(&mut self, depth: u32) { self.ctx.rp.probe_depth = depth; }
    ...
    pub(crate) fn is_def_eq(&mut self, t: Clo<'t>, s: Clo<'t>) -> bool {
        ...
        if let (Some(a), Some(b)) = (self.global_key(tk), self.global_key(sk)) {
            ...
            let r = self.is_def_eq_clo(tk, sk);
            if r {
                self.ctx.rp.g_eq_pos.union(a, b);
            } else if self.in_probe() {
        ...
        if self.ctx.rp.eq_neg.contains(&pk)
            || (self.in_probe() && self.ctx.rp.probe_neg.contains(&pk))
        {
            self.ctx.rp.ctrs[6] += 1;
            return false;
        }
    ...
    fn bool_true_by_evaluation(&mut self, tn: &SClo<'t>, sn: &SClo<'t>) -> bool {
        ...
        if !self.ctx.eager_mode && self.sclo_has_fvar(tn) {
            return false;
        }
        let w = self.whnf_sclo(tn.clone());
        w.spine.is_empty()
            && matches!(self.ctx.read_expr(w.head.e), Const { name, .. } if name == bool_true)
    }

    fn is_def_eq_s_core(&mut self, tn: SClo<'t>, sn: SClo<'t>) -> bool {
        if let Some(b) = self.quick_heads(&tn, &sn) {
            return b;
        }
        if self.bool_true_by_evaluation(&tn, &sn) {
            return true;
        }
        if let Some(b) = self.def_eq_offset(&tn, &sn) {
            return b;
        }
        // proof irrelevance
        ...
        match self.lazy_delta(tn, sn, 10_000) {
            Ok(b) => b,
            Err((tn, sn)) => {
                if let Some(b) = self.quick_heads(&tn, &sn) {
                    return b;
                ...
                match self.ctx.read_expr_pair(tn.head.e, sn.head.e) {
                    (Const { name: c1, levels: l1, .. }, Const { name: c2, levels: l2, .. }) => {
                        if c1 == c2 && self.lvl_eq_list(l1, l2) && self.def_eq_spines(&tn, &sn)
                        {
                            return true;
                        }
                    }
                    (
                        ...
                        let heads_eq = tn.head.e == sn.head.e
                            || (i1 == i2 && self.is_def_eq(Clo::of(t1), Clo::of(t2)));
                        if heads_eq && self.def_eq_spines(&tn, &sn) {
                            return true;
                        }
                    }
                    (
                ...
                if self.try_eta_expansion(&tn, &sn) {
                    return true;
                }
                if self.try_eta_struct(&tn, &sn) {
                    return true;
                }
                if self.def_eq_string_lit(&tn, &sn) {
                    return true;
                }
                if self.def_eq_unit_like(&tn, &sn) {
                    return true;
                }
    ...
    fn def_eq_spines(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> bool {
    ...
    fn quick_heads(&mut self, tn: &SClo<'t>, sn: &SClo<'t>) -> Option<bool> {
        match self.ctx.read_expr_pair(tn.head.e, sn.head.e) {
            (Lambda { .. }, Lambda { .. }) | (Pi { .. }, Pi { .. }) => {
                if tn.spine.is_empty() && sn.spine.is_empty() {
                    return Some(self.def_eq_binding(tn.head, sn.head));
                }
                None
            }
            (Sort { level: l1, .. }, Sort { level: l2, .. }) => Some(self.lvl_eq1(l1, l2)),
    ...
    fn def_eq_binding(&mut self, t: Clo<'t>, s: Clo<'t>) -> bool {
        let (td, tb, sd, sb) = match self.ctx.read_expr_pair(t.e, s.e) {
            ...
            _ => return self.is_def_eq(t, s),
        };
        let tdc = Clo { e: td, env: t.env };
        let sdc = Clo { e: sd, env: s.env };
        if !self.is_def_eq(tdc, sdc) {
            return false;
        }
        let d = self.reify(sdc);
        let level = self.next_level(t.e, t.env).max(self.next_level(s.e, s.env));
        let fv = self.fvar_at(level, d);
        let tenv = self.push_entry(t.env, Entry::Neu(fv));
        let senv = self.push_entry(s.env, Entry::Neu(fv));
        self.def_eq_binding(Clo { e: tb, env: tenv }, Clo { e: sb, env: senv })
    }
    ...
    fn clo_has_fvar(&mut self, e: ExprPtr<'t>, env: EnvId) -> bool {
        self.clo_has_fvar_go(e, env, 0)
    }

    fn clo_has_fvar_go(&mut self, e: ExprPtr<'t>, env: EnvId, offset: u16) -> bool {
        ...
        let r = match n {
            ...
            App { fun, arg, .. } => {
                self.clo_has_fvar_go(fun, env, offset)
                    || self.clo_has_fvar_go(arg, env, offset)
            }
            Lambda { binder_type, body, .. } | Pi { binder_type, body, .. } => {
                self.clo_has_fvar_go(binder_type, env, offset)
                    || self.clo_has_fvar_go(body, env, offset + 1)
            }
            Let { binder_type, val, body, .. } => {
                self.clo_has_fvar_go(binder_type, env, offset)
                    || self.clo_has_fvar_go(val, env, offset)
                    || self.clo_has_fvar_go(body, env, offset + 1)
            }
            Proj { structure, .. } => self.clo_has_fvar_go(structure, env, offset),
            _ => false,
        };
    ...
    fn sclo_has_fvar(&mut self, s: &SClo<'t>) -> bool {
        if self.clo_has_fvar(s.head.e, s.head.env) {
            return true;
        }
        for &c in s.spine.iter() {
            if self.clo_has_fvar(c.e, c.env) {
                return true;
            }
        }
        false
    ...
    fn nat_succ_of(&mut self, t: &SClo<'t>) -> Option<SClo<'t>> {
        ...
        {
            Some(self.mk_sclo(t.spine[0]))
        } else {
            None
        }
    }

    fn def_eq_offset(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> Option<bool> {
        if self.nat_zero_s(t) && self.nat_zero_s(s) {
            return Some(true);
        }
        if let (Some(t2), Some(s2)) = (self.nat_succ_of(t), self.nat_succ_of(s)) {
            return Some(self.is_def_eq_s(t2, s2));
        }
        None
    }
    ...
    fn lazy_delta(
        ...
        if (self.nat_op(&tn).is_some() || self.nat_op(&sn).is_some())
            && (self.ctx.eager_mode
                || (!self.sclo_has_fvar(&tn) && !self.sclo_has_fvar(&sn)))
        {
            if let Some(t2) = self.reduce_nat(&tn) {
                return Ok(self.is_def_eq_s(t2, sn));
            }
            if let Some(s2) = self.reduce_nat(&sn) {
                return Ok(self.is_def_eq_s(tn, s2));
            }
        }
        self.check_native(&tn);
        self.check_native(&sn);
        let dt = self.delta_hint(tn.head.e);
        let ds = self.delta_hint(sn.head.e);
        match (dt, ds) {
            (None, None) => Err((tn, sn)),
            (Some(_), None) => {
                if let Some(s2) = self.try_unfold_proj_app(&sn) {
                    return self.lazy_delta(tn, s2, fuel - 1);
                }
                let t2 = self.delta1(&tn);
                self.lazy_delta(t2, sn, fuel - 1)
            }
            (None, Some(_)) => {
                if let Some(t2) = self.try_unfold_proj_app(&tn) {
                    return self.lazy_delta(t2, sn, fuel - 1);
                }
                let s2 = self.delta1(&sn);
                self.lazy_delta(tn, s2, fuel - 1)
            }
            (Some(ht), Some(hs)) => {
                if ht.is_lt(&hs) {
                    let s2 = self.delta1(&sn);
                    ...
                    if !tn.spine.is_empty() && !sn.spine.is_empty() {
                        ...
                        if same_const {
                            let saved = self.probe_enter(SPEC_BUDGET);
                            let r = self.def_eq_spines(&tn, &sn);
                            let aborted = self.probe_exit(saved);
                            if r && !aborted {
                                return Ok(true);
                            }
                            if aborted {
    ...
    fn try_unfold_proj_app(&mut self, s: &SClo<'t>) -> Option<SClo<'t>> {
        if !matches!(self.ctx.read_expr(s.head.e), Proj { .. }) {
            return None;
        }
        let r = self.whnf_core(s.clone());
    ...
    fn delta1(&mut self, x: &SClo<'t>) -> SClo<'t> {
        let u = self.unfold_definition(x).expect("delta on non-unfoldable");
        self.whnf_core_ext(u, false, true)
    }

    fn check_native(&self, s: &SClo<'t>) {
    ...
    fn try_eta_expansion(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> bool {
        if self.try_eta_core(t, s) {
            return true;
        }
        self.try_eta_core(s, t)
    }

    fn try_eta_core(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> bool {
        let t_is_lam =
        ...
        let s_ty = self.infer_s(s, InferOnly);
        let s_ty_w = self.whnf_clo(Clo::of(s_ty));
        ...
        let d = self.reify(Clo { e: dom, env: s_ty_w.head.env });
        let level = self
            .next_level(t.head.e, t.head.env)
            .max(self.next_level(s.head.e, s.head.env))
            .max(self.max_level(d));
        let fv = self.fvar_at(level, d);
        let Lambda { body: t_body, .. } = self.ctx.read_expr(t.head.e) else {
            return false;
        };
        let tenv = self.push_entry(t.head.env, Entry::Neu(fv));
        let mut s_applied = s.clone();
        s_applied.spine.push(Clo::of(fv));
        let lhs = self.mk_sclo(Clo { e: t_body, env: tenv });
        self.is_def_eq_s(lhs, s_applied)
    }

    fn try_eta_struct(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> bool {
    ...
    fn try_eta_struct_core(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> bool {
        ...
        let t_ty = self.infer_s(t, InferOnly);
        let s_ty = self.infer_s(s, InferOnly);
        if !self.is_def_eq(Clo::of(t_ty), Clo::of(s_ty)) {
            return false;
        }
        let t_clo = self.sclo_as_clo(t);
        for i in num_params..(num_params + num_fields) {
            let bv = self.ctx.mk_var(0);
            let proj = self.ctx.mk_proj(induct, (i - num_params) as usize, bv);
            let env = self.push_entry(ENV_NIL, Entry::Val(t_clo.e, t_clo.env));
            if !self.is_def_eq(Clo { e: proj, env }, s.spine[i as usize]) {
                return false;
            }
        }
        true
    ...
    fn def_eq_string_lit(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> bool {
        fn core<'x, 't: 'x, 'p: 't>(
            ...
            let lhs = ck.mk_sclo(Clo::of(e));
            ck.is_def_eq_s(lhs, s.clone())
        }
        if core(self, t, s) {
            return true;
        }
    ...
    fn def_eq_unit_like(&mut self, t: &SClo<'t>, s: &SClo<'t>) -> bool {
        let t_ty = self.infer_s(t, InferOnly);
        let ty_w = self.whnf_clo(Clo::of(t_ty));
        ...
        let ty_clo = self.sclo_as_clo(&ty_w);
        let s_ty = self.infer_s(s, InferOnly);
        self.is_def_eq(ty_clo, Clo::of(s_ty))
    }
    ...
    pub(crate) fn sclo_to_expr(&mut self, s: &SClo<'t>) -> ExprPtr<'t> {
        let mut out = self.reify(s.head);
        for &c in s.spine.iter() {
            let a = self.reify(c);
